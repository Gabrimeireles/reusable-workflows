...
`version` (extracted version string, or empty).
...
**Outputs**: `health_status` (`success`/`failure`/`skipped`), `deployment_time_seconds`.
...
`version_file` (default `./package.json`), `prerelease` (default `false`).
